...
9. [Version History (Git Strategy)](#version-history-git-strategy)
...
├── test_app.py                   # Pytest test suite (50+ test cases)
...
**Expected output:** 50+ tests, all passing, ≥70% coverage.
...
1. Install Jenkins (local or Docker):
   ...
   docker run -d -p 8080:8080 -p 50000:50000 \
     ...
     jenkins/jenkins:lts
   ```
2. In the Jenkins UI, install the **Pipeline** and **JUnit** plugins.
3. Create a new **Pipeline** project → set SCM to your GitHub repo → set Script Path to `Jenkinsfile`.
4. Ensure the Jenkins agent has **Python 3**, **pip**, **Docker**, and **curl** available.
...
| **Lint** | Runs `flake8` — fails the build on syntax/undefined-name errors |
| **Unit Tests** | Runs `pytest` with JUnit XML output, published to Jenkins |
| **Docker Build** | Builds the production Docker image and tags it with the build number |
| **Docker Smoke Test** | Spins up the container and hits `/health` to verify it starts correctly |
| **Quality Gate** | Parses `coverage.xml` and fails the build if coverage drops below **70%** |
...
| GET | `/clients/<name>` | Get a client |
| PUT | `/clients/<name>` | Update a client |
| DELETE | `/clients/<name>` | Delete a client |
| GET | `/clients/<name>/calories` | Get calorie target |
...
test: add 50+ pytest cases covering CRUD, validation, calories
